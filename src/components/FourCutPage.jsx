...
function drawStickers(ctx, landmarks, filter, stickers, w, h) {
  ...
  if (filter === "bunny" && stickers.bunny) {
    const eW = eyeW * 2;
    const eH =
      eW * (stickers.bunny.naturalHeight / stickers.bunny.naturalWidth);
    const fx = forehead.x * w,
      fy = forehead.y * h - eH * 1.1;
    ctx.save();
    ctx.translate(fx, fy + eH / 2);
    ctx.rotate(angle);
    ctx.drawImage(stickers.bunny, -eW / 2, -eH / 2, eW, eH);
    ctx.restore();
  }
  ...
  if (filter === "heart" && stickers.heart) {
    const hS = eyeW * 0.72;
    const lc = {
      x: leftEye.x * w - eyeW * 0.18,
      y: leftEye.y * h + eyeW * 0.62,
    };
    const rc = {
      x: rightEye.x * w + eyeW * 0.18,
      y: rightEye.y * h + eyeW * 0.62,
    };
    ctx.save();
    ctx.translate(lc.x, lc.y);
    ctx.rotate(angle - 0.18);
    ctx.drawImage(stickers.heart, -hS / 2, -hS / 2, hS, hS);
    ctx.restore();
    ctx.save();
...
const CamWrap = styled.div`
  ...
  background: #111;
  overflow: hidden;
  box-shadow: 0 14px 32px rgba(31, 26, 26, 0.18);
`;
...
const Spinner = styled.div`
  ...
  height: 28px;
  border-radius: 50%;
  border: 3px solid rgba(255, 253, 242, 0.22);
...
const StatusOverlay = styled.div`
  ...
  gap: 12px;
  background: ${({ $error }) =>
    $error ? "rgba(80,20,20,.88)" : "rgba(22,18,18,.78)"};
  color: #fffdf2;
...
const FlashLayer = styled.div`
  ...
  z-index: 10;
  pointer-events: none;
  animation: ${flashKf} 0.35s ease-out forwards;
`;
...
const Chip = styled.button`
  ...
  ${({ $active }) =>
    $active &&
    css`
      animation: ${filterPop} 0.22s ease-out;
    `}
`;
...
export default function FourCutPage({ onNext }) {
  ...
  /* Phase C: render loop */
  useEffect(() => {
    if (phase === "loading" || phase === "done" || phase === "composing")
      return;
    let cancelled = false;
    const v = videoRef.current,
    ...
    const loop = () => {
      ...
      if (v.readyState >= 2 && v.videoWidth) {
        ...
        if (lm) {
          try {
            lastLmRef.current =
              lm.detectForVideo(v, performance.now())?.faceLandmarks ?? [];
          } catch (_) {
            /* keep last */
          }
          lastLmRef.current.forEach((lms) =>
            drawStickers(
              ctx,
              lms,
              filterRef.current,
      ...
      <Stage>
          ...
          {showCam ? (
            <CamWrap>
              ...
              {isCapturing && countdown !== null && (
                <CamCountdown>
                  <ShotBadge>
                    {shotIndex + 1} / {SHOT_COUNT}
                  </ShotBadge>
                  <CountNum
                    key={`${shotIndex}-${countdown}`}
                    $shoot={countdown === 0}
                  >
            ...
            <FrameWrap>
              ...
              <FrameInner>
                ...
                {composedSrc && (
                  <img className="composed" src={composedSrc} alt="인생네컷" />
                )}
              </FrameInner>
        ...
        <ControlPane>
          ...
          {isReady && (
            <>
              <Desc>
                시작하면 {COUNTDOWN_SEC}초 카운트다운이 시작돼요.
                <br />매 컷마다 필터를 바꿀 수 있으니 준비하세요!
              </Desc>
              <FilterRow>
                {FILTERS.map((f) => (
                  <Chip
          ...
          {isCapturing && (
            <CaptureStatus key={shotIndex}>
              <StatusLine>
                {shotIndex + 1} / {SHOT_COUNT}번째 촬영 중
              </StatusLine>
              ...
              {countdown === null && (
                <StatusLine>다음 컷 준비 중...</StatusLine>
              )}
            </CaptureStatus>
          )}

          {isComposing && <StatusLine>합성 중...</StatusLine>}
